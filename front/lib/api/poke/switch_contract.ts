...
// AWU usage cap is denominated in AWU credits, written directly to
// `credit_usage_configuration.usageCapCredits`. We deliberately do NOT accept
// dollars/euros here and do NOT convert from any fiat unit —
// `programmatic_usage_configuration.paygCapMicroUsd` (micro_usd) is a legacy
// concern of the programmatic-usage flow and must stay out of this path. The
// usage cap is independent from `paygEnabled`; either can be set on its own.
export const MAX_USAGE_CAP_CREDITS = 2_000_000;
...
export const SwitchContractBodySchema = z.object({
  ...
  usageCapCredits: z
    ...
    .max(
      MAX_USAGE_CAP_CREDITS,
      `Usage cap cannot exceed ${MAX_USAGE_CAP_CREDITS.toLocaleString()} credits`
    )
    .optional(),
});
